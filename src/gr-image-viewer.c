...
{
        if (((GdkEventCrossing *)event)->detail == GDK_NOTIFY_INFERIOR)
                        return FALSE;
...
gr_image_viewer_init (GrImageViewer *self)
{
        ...
        g_signal_connect (self, "enter-notify-event", G_CALLBACK (enter_leave_notify), self);
        g_signal_connect (self, "leave-notify-event", G_CALLBACK (enter_leave_notify), self);
        g_signal_connect (self, "motion-notify-event", G_CALLBACK (motion_notify), self);
